...
def preprocess(save_path,token):
    token_attn_map = token.detach().cpu().numpy()
...
def save_all_token_attn_maps(maps,id_to_word):

    for i,token_maps in enumerate(maps):
        if i==0:
            continue
        token_path = id_to_word[i] + '.jpg'
        base_path = '/mnt/task_runtime/Linguistic-Binding-in-Diffusion-Models/debug_output'
        save_path = os.path.join(base_path,token_path)
        preprocess(save_path,token_maps)
        
class GOMEDiffusionPipeline(StableDiffusionPipeline):
    def __init__(self,
                 vae: AutoencoderKL,
    ...
    def __call__(
            ...
            tenor: Union[str, List[str]] = None,
            vehicle: Union[str, List[str]] = None,
        ...
        Args:
            ...
            binding_step_size (`float`, *optional*, default to 20.0):
                Controls the step size of each GOME update.
            num_intervention_steps ('int', *optional*, defaults to 25):
                The number of times we apply GOME.
    ...
    def _attribution_loss(
        ...
        if not self.subtrees_indices:
            self.subtrees_indices = self._extract_binding_pairs(prompt)
    ...
    def _select_pairs(self, binding_pair_list):
        final_pairs = []
        
        if self.tenor: 
            target_words = self.tenor if type(self.tenor) == list else [self.tenor]
            for pair in binding_pair_list:
                
                low_pair = [word.lower_ for word in pair]
                for meta in target_words:
                    is_save = False
                    for word in low_pair:
                        if string_similarity(meta,word) > 0.4:
                            is_save = True
                            break
                    if is_save == True:
        ...
        if len(final_pairs) != 0:
            ...
            return binding_pair_list
            
    def _extract_binding_pairs(self, prompt):
        binding_indices = []
        binding_sets =[]
        # extract standard attribution indices
        binding_sets_1 = extract_attribution_indices(self.doc)
        if binding_sets_1:
            binding_sets.append(binding_sets_1)
           
        # extract attribution indices with verbs in between
        binding_sets_2 = extract_attribution_indices_with_verb_root(self.doc)
         
        if binding_sets_2:
            binding_sets.append(binding_sets_2)
           
        binding_sets_3 = extract_attribution_indices_with_verbs(self.doc)
        if binding_sets_3:
            binding_sets.append(binding_sets_3)

        binding_sets = unify_lists(binding_sets) 
        final_sets = self._select_pairs(binding_sets)
        # make sure there are no duplicates
        # binding_indices = unify_lists(binding_indices)
        print(f"collected objects-attribute pairs:{final_sets}")
        binding_indices = self._align_indices(prompt, final_sets)
